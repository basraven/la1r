...
**Model catalog (2026-08-23):** codex reads `model_catalog_json = "~/.codex/model-catalogs/models.json"` (top-level config.toml key, Layer-3 override over bundled/remote catalogs). The deepseek-v4-flash entry carries `context_window/max_context_window: 1000000`, `auto_compact_token_limit` (lowered to **600000** on 2026-08-23 to prevent context-window overflow — see [[codex-stream-disconnect-litellm]]), `truncation_policy`. **The catalog JSON schema in codex 0.149.0 has NO max_output_tokens field** (adding one is ignored — `codex debug models` shows it as None); the only output-related knob is the compaction threshold. `codex debug models` prints the resolved metadata. init.sh now has an `ensure_codex_catalog` block that writes/patches the catalog idempotently + ensures `model_catalog_json` in config.toml (durable across PVC rebuilds).
